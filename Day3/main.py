...
class SpecialCharacter:
    ...
    def erase_digit(self, x, y, grid):
        grid[y] = grid[y][:x] + 'X' + grid[y][x+1:]
        return grid

    def recurse_digits(self, x, y, grid):
        digit = grid[y][x]
        grid = self.erase_digit(x, y, grid)
        left = self.recurse_left(x-1, y, '', grid)
        right = self.recurse_right(x+1, y, '', grid)
        digit = left + digit + right
        return int(digit)

    # Step left and find all digits
    def recurse_left(self, x, y, digit, grid):
        if(x >= 0 and re.search("\\d", grid[y][x])):
            digit = grid[y][x] + digit
            grid = self.erase_digit(x, y, grid)
            return self.recurse_left(x-1, y, digit, grid)
        return digit

    # step right and find all digits
    def recurse_right(self, x, y, digit, grid):
        if(x < len(grid[y]) and re.search("\\d", grid[y][x])):
            digit = digit + grid[y][x]
            grid = self.erase_digit(x, y, grid)
